...
async def ocr_utility_bill(
    ...
    try:
        if kind == "pdf":
            ...
            texts = [pytesseract.image_to_string(im)]
    except Exception as e:  # noqa: BLE001
        log.error("ocr_error", error=str(e))
        raise HTTPException(400, f"OCR failed: {e}") from e

    fields = candidates_from_texts(texts)
    out = {"tables": [], "fields": [{"name": k, "candidates": v} for k, v in fields.items()]}
    log.info("ocr_done", pages=len(texts), kwh=len(fields.get("kWh", [])), date=len(fields.get("date", [])))
    return out
